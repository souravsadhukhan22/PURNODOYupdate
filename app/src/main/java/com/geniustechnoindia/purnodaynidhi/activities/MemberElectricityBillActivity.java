...
public class MemberElectricityBillActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {
    // toolbar
    ...
    private TextView mTv_rechargeAmount;
    ...
    private TextView mTv_billDate,mTv_billDueDate,mTv_custName,mTv_partialPay;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadElectricityBillOpCodes(APILinks.GET_OPERATOR_CODES_NEW + "Electricity%20Bill");
    }
    ...
    private void setupToolbar() {
        ...
        mTv_toolbarTitle.setText("Electricity Bill");
    }

    private void getMenuOptions(String url) {
        new GetDataParserArray(MemberElectricityBillActivity.this, url, true, new GetDataParserArray.OnGetResponseListener() {
            @Override
                ...
                if (response != null) {
                    ...
                    try {
                        if (response.length() > 0) {
                            ...
                            Toast.makeText(MemberElectricityBillActivity.this, "No savings account found", Toast.LENGTH_SHORT).show();
                        }
    ...
    private void getCurrentBalance(String accCode) {
        new GetDataParserArray(MemberElectricityBillActivity.this, APILinks.GET_SB_CURRENT_BALANCE + accCode, true, new GetDataParserArray.OnGetResponseListener() {
            @Override
            public void onGetResponse(JSONArray response) {
                if (response != null) {
                    if (response.length() > 0) {
    ...
    private void loadElectricityBillOpCodes(String url) {
        new GetDataParserArray(MemberElectricityBillActivity.this, url, true, response -> {
            if (response != null) {
                if (response.length() > 0) {
                    for (int i = 0; i < response.length(); i++) {
                    ...
                    mSs_providerSearchableSpinner.setAdapter(new ArrayAdapter<ProviderNewSetGet>(MemberElectricityBillActivity.this, android.R.layout.simple_spinner_dropdown_item, mArrayListProviderSetGet));
                }
            }
        });
    }
                ...
                if (mTie_customerNumber.getText().toString().trim().length() > 0) {
                    if (!providerId.equals("")) {
                        ...
                        if (mTie_mobileNumber.getText().toString().trim().length() > 0) {
                            if (mTv_rechargeAmount.getText().toString().trim().length() > 0) {
                                memberCode = GlobalStore.GlobalValue.getUserName();
                                amount = mTv_rechargeAmount.getText().toString();
                                ...
                                payBill(MemberElectricityBillActivity.this, APILinks.PAY_BILL, hashMap);
                            } else {
                                mTv_rechargeAmount.setError("Enter recharge amount");
                                mTv_rechargeAmount.requestFocus();
                            }
                        } else {
                    ...
                    if (response.getBoolean("APIStatus")) {
                        if (!response.getBoolean("IsBalanceLow")) {
                            if (response.getBoolean("IsRechargeRecorded")) {
                                showConfirmationDialog("Success", "Your request for Electricity bill payment has been recorded successfully.", true);
                            } else {
                                showConfirmationDialog("Failed", "Failed to record your Electricity bill payment request.", false);
                            }
                        } else {
    ...
    private void showConfirmationDialog(String title, String message, boolean status) {
        AlertDialog.Builder builder = new AlertDialog.Builder(MemberElectricityBillActivity.this);
        builder.setTitle(title);
        builder.setMessage(message);
        builder.setPositiveButton("Ok", ((dialog, which) -> {
            dialog.dismiss();
            if (status) {
                sendSmsConfirmation(mTv_rechargeAmount.getText().toString(), GlobalStore.GlobalValue.getPhone());
            }
        })).show();
    }
    ...
    private void sendSmsConfirmation(String rechargeAmt, String phoneNumber) {
        msgBody = "Dear Customer,Electricity bill payment of Rs- " + rechargeAmt + " is successful and debited from your savings account, " + R.string.App_Full_Name +".";
        ...
        client.get(smsUrl, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                startActivity(new Intent(MemberElectricityBillActivity.this, MemberElectricityBillActivity.class));
    ...
    private void setViewReferences() {
        ...
        mTv_rechargeAmount = findViewById(R.id.tv_activity_electricity_bill_recharge_amount);
        ...
        mTv_partialPay = findViewById(R.id.tv_activity_member_electricity_bill_partial_pay_allowed);
        ...
        mTie_mobileNumber = findViewById(R.id.tie_activity_member_electricity_bill_mobile_no);

        mTv_billDate = findViewById(R.id.tv_activity_member_electricity_bill_date);
        mTv_billDueDate = findViewById(R.id.tv_activity_member_electricity_bill_due_date);
        mTv_custName = findViewById(R.id.tv_activity_member_electricity_bill_cust_name);
    }
    ...
    public void onBackPressed() {
        //super.onBackPressed();
        startActivity(new Intent(MemberElectricityBillActivity.this, MemberDashboardActivity.class));
        overridePendingTransition(R.anim.fade_in, R.anim.fade_out);
        finish();
    }
        ...
        if (!providerId.equals("")) {
            if (mTie_customerNumber.getText().toString().trim().length() > 0) {
                if (mTie_mobileNumber.getText().toString().trim().length() > 0) {
                    ...
                    hashMap.put("LAT", "23.3017");
                    hashMap.put("LONG", "88.5302");
                    callFetchBill(MemberElectricityBillActivity.this, APILinks.FETCH_BILL, hashMap);
                } else {
                    mTie_mobileNumber.setError("Enter reg. mobile no.");
                    mTie_mobileNumber.requestFocus();
                }
            } else {
                mTie_customerNumber.setError("Enter customer no");
                mTie_customerNumber.requestFocus();
            }
        } else {
            Toast.makeText(this, "Select provider", Toast.LENGTH_SHORT).show();
    ...
    private void callFetchBill(Context context, String URL, HashMap<String, String> hashMap) {
        new PostDataParserObjectResponse(context, URL, hashMap, true, response -> {
            if (response != null) {
                /*
        *
        * {
            ...
            "STATUSMSG": "Success",
            "BILLNO": "538002257871",
            "CUSTNAME": "BHANDUL  SARDER",
            "BILLDATE": "24-02-2020",
            "BILLDUEDATE": "15-05-2020",
            "PARTIALPAYALLOW": "NO",
            "BILLAMT": 129.0
           }
        *
        * */
                ...
                try {
                    if (response.getString("STATUSCODE").equals("0")) {
                        if (response.getString("STATUSMSG").equals("Success")) {
                            mTv_rechargeAmount.setText(String.valueOf(response.getInt("BILLAMT")));
                            mTv_billDate.setText(response.getString("BILLDATE"));
                            mTv_billDueDate.setText(response.getString("BILLDUEDATE"));
                            mTv_custName.setText(response.getString("CUSTNAME"));
                            mTv_partialPay.setText(response.getString("PARTIALPAYALLOW"));
                        } else {
                            Toast.makeText(context, "Failed to fetch bill", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        showBillMessage(response.getString("STATUSMSG"));
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
    ...
    private void showBillMessage(String msg){
        AlertDialog.Builder builder = new AlertDialog.Builder(MemberElectricityBillActivity.this);
        builder.setMessage(msg);
        builder.setPositiveButton("Ok", (dialog, which) -> {
            mTv_rechargeAmount.setText("");
            mTv_billDate.setText("");
            mTv_billDueDate.setText("");
            mTv_custName.setText("");
            mTv_partialPay.setText("");
            dialog.dismiss();
        }).show();
