...
public class MemberDTHRechargeNewActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {
    ...
    private void setupToolbar() {
        ...
        mTv_toolbarTitle.setText("DTH Recharge");
    }

    private void getMenuOptions(String url) {
        new GetDataParserArray(MemberDTHRechargeNewActivity.this, url, true, new GetDataParserArray.OnGetResponseListener() {
            @Override
                ...
                if (response != null) {
                    ...
                    try {
                        if (response.length() > 0) {
                            ...
                            Toast.makeText(MemberDTHRechargeNewActivity.this, "No savings account found", Toast.LENGTH_SHORT).show();
                        }
    ...
    private void getCurrentBalance(String accCode){
        new GetDataParserArray(MemberDTHRechargeNewActivity.this, APILinks.GET_SB_CURRENT_BALANCE + accCode, true, new GetDataParserArray.OnGetResponseListener() {
            @Override
            public void onGetResponse(JSONArray response) {
                if(response != null){
                    if(response.length() > 0){
    ...
    private void loadElectricityBillOpCodes(String url) {
        new GetDataParserArray(MemberDTHRechargeNewActivity.this, url, true, response -> {
            if (response != null) {
                if (response.length() > 0) {
                    for (int i = 0; i < response.length(); i++) {
                    ...
                    mSs_providerSearchableSpinner.setAdapter(new ArrayAdapter<ProviderNewSetGet>(MemberDTHRechargeNewActivity.this, android.R.layout.simple_spinner_dropdown_item, mArrayListProviderSetGet));
                }
            }
        });
    }
                    ...
                    if(mTie_mobileNumber.getText().toString().trim().length() > 0){
                        if (!providerId.equals("")) {
                            if (mTie_rechargeAmount.getText().toString().trim().length() > 0) {
                                ...
                                payBill(MemberDTHRechargeNewActivity.this,APILinks.PAY_BILL,hashMap);
                            } else {
    ...
    private void showConfirmationDialog(String title, String message, boolean status) {
        AlertDialog.Builder builder = new AlertDialog.Builder(MemberDTHRechargeNewActivity.this);
    ...
    private void sendSmsConfirmation(String rechargeAmt, String phoneNumber) {
        msgBody = "Dear Customer,DTH recharge of Rs- " + rechargeAmt + " is successful and debited from your savings account, " + R.string.App_Full_Name +".";
        ...
        client.get(smsUrl, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                startActivity(new Intent(MemberDTHRechargeNewActivity.this, MemberDTHRechargeNewActivity.class));
    ...
    private void setViewReferences() {
        ...
        mTie_mobileNumber = findViewById(R.id.tie_activity_member_dth_recharge_new_mobile_number);
    ...
    public void onBackPressed() {
        //super.onBackPressed();
        startActivity(new Intent(MemberDTHRechargeNewActivity.this, MemberDashboardActivity.class));
        overridePendingTransition(R.anim.fade_in, R.anim.fade_out);
        finish();
    }
