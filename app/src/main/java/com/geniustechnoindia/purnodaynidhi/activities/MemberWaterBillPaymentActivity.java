...
public class MemberWaterBillPaymentActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemSelectedListener {
    // toolbar
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loadElectricityBillOpCodes(APILinks.GET_OPERATOR_CODES_NEW + "Water%20Bill");
    }
    ...
    private void setupToolbar() {
        ...
        mTv_toolbarTitle.setText("Water Bill");
    }

    private void getMenuOptions(String url) {
        new GetDataParserArray(MemberWaterBillPaymentActivity.this, url, true, new GetDataParserArray.OnGetResponseListener() {
            @Override
                ...
                if (response != null) {
                    ...
                    try {
                        if (response.length() > 0) {
                            ...
                            Toast.makeText(MemberWaterBillPaymentActivity.this, "No savings account found", Toast.LENGTH_SHORT).show();
                        }
    ...
    private void getCurrentBalance(String accCode){
        new GetDataParserArray(MemberWaterBillPaymentActivity.this, APILinks.GET_SB_CURRENT_BALANCE + accCode, true, new GetDataParserArray.OnGetResponseListener() {
            @Override
            public void onGetResponse(JSONArray response) {
                if(response != null){
                    if(response.length() > 0){
    ...
    private void loadElectricityBillOpCodes(String url) {
        new GetDataParserArray(MemberWaterBillPaymentActivity.this, url, true, response -> {
            if (response != null) {
                if (response.length() > 0) {
                    for (int i = 0; i < response.length(); i++) {
                    ...
                    mSs_providerSearchableSpinner.setAdapter(new ArrayAdapter<ProviderNewSetGet>(MemberWaterBillPaymentActivity.this, android.R.layout.simple_spinner_dropdown_item, mArrayListProviderSetGet));
                }
            }
        });
    }
                        ...
                        if(mTie_mobileNumber.getText().toString().trim().length() > 0){
                            if (mTie_rechargeAmount.getText().toString().trim().length() > 0) {
                                ...
                                if(mTie_remarks.getText().toString().trim().length() > 0){
                                    ...
                                    hashMap.put("Reamrks", "Water bill from SB");
                                }
                                ...
                                payBill(MemberWaterBillPaymentActivity.this,APILinks.PAY_BILL,hashMap);
                            } else {
                    ...
                    if (response.getBoolean("APIStatus")) {
                        if (!response.getBoolean("IsBalanceLow")) {
                            if (response.getBoolean("IsRechargeRecorded")) {
                                showConfirmationDialog("Success", "Your request for Water bill payment has been recorded successfully.", true);
                            } else {
                                showConfirmationDialog("Failed", "Failed to record your Water bill payment request.", false);
                            }
                        } else {
    ...
    private void showConfirmationDialog(String title, String message, boolean status) {
        AlertDialog.Builder builder = new AlertDialog.Builder(MemberWaterBillPaymentActivity.this);
    ...
    private void sendSmsConfirmation(String rechargeAmt, String phoneNumber) {
        msgBody = "Dear Customer,Water bill payment of Rs- " + rechargeAmt + " is successful and debited from your savings account, "+ R.string.App_Full_Name +".";
        ...
        client.get(smsUrl, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                startActivity(new Intent(MemberWaterBillPaymentActivity.this, MemberWaterBillPaymentActivity.class));
    ...
    public void onBackPressed() {
        //super.onBackPressed();
        startActivity(new Intent(MemberWaterBillPaymentActivity.this, MemberDashboardActivity.class));
        overridePendingTransition(R.anim.fade_in, R.anim.fade_out);
        finish();
    }
